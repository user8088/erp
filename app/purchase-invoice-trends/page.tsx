...
export default function PurchaseInvoiceTrendsPage() {
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      console.error(e);
      const message = e instanceof Error ? e.message : "Failed to load Purchase Invoice Trends.";
      setError(message);
  ...
  return (
    <div className="max-w-7xl mx-auto min-h-full py-4">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Purchase Invoice Trends</h1>
        <p className="mt-1 text-sm text-gray-500">
          Analyze expense trends over time from purchase invoices.
        </p>
      </div>
      ...
      {report && (
        <div className="space-y-6">
          ...
          {report.data_points.length > 0 && (
            <TrendChart data={report.data_points} metric="expense" trend={report.trend} />
          )}

          {/* Trends Table */}
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h2 className="text-lg font-semibold text-gray-900">Expense Trends</h2>
            </div>
